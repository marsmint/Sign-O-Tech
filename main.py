...
ROI_right = 150
ROI_left = 350
directory = r"C:\Users\kit\PycharmProjects\pythonProject\gestures\train\8"


def cal_accum_avg(frame1, accumulatedweight):
    global background

    if background is None:
        background = frame1.copy().astype("float")
        return None

    cv2.accumulateWeighted(frame1, background, accumulatedweight)


def segment_hand(frame2, threshold=25):
    global background

    diff = cv2.absdiff(background.astype("uint8"), frame2)
...
while True:
    ...
    '''else:
        ...
        if hand is not None:
            ...
            if num_imgs_taken <= 300:
                ...
                cv2.imwrite(directory + "\\" + str(num_imgs_taken) + '.jpg', thresholded)
            else:
                break
            num_imgs_taken += 1
        else:
    ...
    cv2.putText(frame_copy, "Sign-O-Tech", (10, 20), cv2.FONT_ITALIC, 0.5, (51, 255, 51), 1)
